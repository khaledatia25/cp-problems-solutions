...
void elmtarshm(int tc) {
    ...
    int dp[n + 1][3];  // dp[i][activity] is max points of happiness can be achieved from day one to day i when the cur activity is the "activity"
    ...
    for (int i = 1; i <= n; i++) {
        for (int j = 0; j < 3; j++) {
            ...
            if (j != 1) {
                dp[i][j] = max(dp[i - 1][1] + b[i], dp[i][j]);
            }
            if (j != 2) {
                dp[i][j] = max(dp[i][j], dp[i - 1][2] + c[i]);
            }
        }
    }
