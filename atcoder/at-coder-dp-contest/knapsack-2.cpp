...
void elmtarshm(int tc) {
    ...
    int w[n], v[n];
    int max_val = 0;
    for (int i = 0; i < n; i++) cin >> w[i] >> v[i], max_val += v[i];
    int dp[max_val + 1];  // dp[i] min weight to optain value i
    for (int i = 0; i <= max_val; i++) dp[i] = OO;
    dp[0] = 0;
    for (int i = 0; i < n; i++) {
        for (int j = max_val; j >= 0; j--) {
            if (v[i] <= j) dp[j] = min(dp[j], dp[j - v[i]] + w[i]);
        }
    }
    for (max_val; max_val >= 0; max_val--) {
        if (dp[max_val] <= max_w) {
            cout << max_val << endl;
            return;
        }
