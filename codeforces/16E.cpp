...
/*
    ...
    in each day there are
        (n - i) c 2 of choices
    dp[day][fish number] is the probability of this fish to stay a live untinl this day
    ...
    for (int day = 1; day < n; day++) {
        for (int first = 0; first < n; first++) {
            for (int second = 0; second < n; second++) {
                if (first == second) {
                    dp[day][first] = dp[day - 1][first];
                    continue;
                }
                double a = 2.0 / (1.0 * (n - day + 1) * (n - day));
                dp[day][first] += a * dp[day - 1][first] * dp[day - 1][second] * prob[first][second];
            }
        }
    }
    prec(9);
