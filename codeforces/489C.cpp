...
void freePalestine(int tc) {
    int n, m;
    cin >> n >> m;
    function<bool(int, int)> can = [&](int a, int b) {
        return b >= 0 && b <= a * 9;
    };
    string smallest, largest;
    if (m == 0 && n == 1) return cout << 0 << space << 0 << endl, void();
    if (m == 0 || !can(n, m)) {
        cout << "-1 -1" << endl;
        return;
    }
    ...
    int temp = m;
    for (int i = 0; i < n; i++) {
        int add = min(9ll, temp);
        largest.push_back('0' + add);
        temp -= add;
    }
    cout << smallest << space << largest << endl;
}
